...
func (b *backend) defrag() error {
	...
	c, err := bolt.NewCompaction(b.db, int64(defragLimit))
	if err != nil {
		return err
	}
	defer c.Cleanup()
	...
	if err := c.CollectFinalChanges(); err != nil {
		b.lg.Error("failed to collect final changes during defrag", zap.Error(err))
	...
	newDB, err := c.Swap()
	if err != nil {
		b.lg.Fatal("failed to swap database during defrag", zap.Error(err))
	}
	b.db = newDB
